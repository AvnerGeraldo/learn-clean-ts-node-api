...
describe('Signup Controller', () => {
  ...
  test('Should return 400 if no password confirmation fails', async () => {
    const { sut } = makeSut() // sut = System under test
    const httpRequest = deepmerge(makeFakeRequest(), { body: { passwordConfirmation: 'invalid_password' } })
    const httpResponse = await sut.handle(httpRequest)
  ...
  test('Should return 400 if an invalid email is provided', async () => {
    ...
    const httpRequest = deepmerge(makeFakeRequest(), { body: { email: 'invalid_email@email.com' } })
    const httpResponse = await sut.handle(httpRequest)
